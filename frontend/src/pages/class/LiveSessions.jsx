      ...
      {activeSession && (
        <section className="rounded-[28px] border border-[#2d5a56]/30 bg-[#0d1f1e] text-white overflow-hidden shadow-[0_20px_50px_rgba(13,31,30,0.3)]">
          ...
          <div className="relative min-h-80 bg-black">
            {(() => {
              // Find the active remote stream from the teacher/host
              const hostPeer = remotePeers.find(
                (p) => (p.id === activeSession?.created_by || ['teacher', 'owner', 'manager'].includes(p.role)) && p.stream
              );
              const activeRemoteStream = hostPeer?.stream || remotePeers.find((p) => p.stream)?.stream || null;
